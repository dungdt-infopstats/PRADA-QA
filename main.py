...
part = 8
type = 'retrieve_rank_web_deepseek'
cut = ''
def parse_arguments():
    ...
    parser.add_argument(
        "--save_path",
        type=str,
        default= f"D:/AI_CODE/MASEE/data/predictions_part{part}_{type}_{cut}.jsonl",
...
def evaluation(
    ...
    with jsonlines.open(save_path, mode='a') as writer:
        for _, row in question_df.iterrows():
            prompt = pqa_instructions.format(row['question_text'], row['question_id'], row['question_type'], row['asin'], row['item_name'], row['description'])

            q_doc, d_doc = q_db.get_by_ids([row['question_id']]), d_db.get_by_ids([row['question_id']])
...
def main():
    ...
    write_json(name, qa_log_dir)
    write_json(name, p_log_dir)
